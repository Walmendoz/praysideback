...
router.get("/buscarusuariocorreo/:email", (req, res) => {
  ...
  conexion.query(
    "SELECT clave, nombre, idioma from prayside_usuarios where email = ?",
    [email],
    (err, rows, fields) => {
      if (err) {
        console.log(err);
...
router.get("/buscardatosusuario/:email", (req, res) => {
  var email = req.params.email;
  conexion.query(
    "SELECT * from prayside_usuarios where email = ?",
    [email],
    (err, rows, fields) => {
      if (err) {
        console.log(err);
...
router.get("/buscarclave/:email", (req, res) => {
  var email = req.params.email;
  conexion.query(
    "SELECT clave, pregunta, respuesta from prayside_usuarios where email = ?",
    [email],
    (err, rows, fields) => {
      if (err) {
        console.log(err);
  ...
  conexion.query(
    ...
    (err, rows, fields) => {
      if (rows.length === 0) {
        ...
        var espacios = " ";
        conexion.query(
          "UPDATE prayside_ciudades set provinciacodigo = ? where paiscodigodos = ? and ciudadcodigo = ? and provinciacodigo = ?",
          [provincia, pais, ciudad, espacios],
          (err, rows, fields) => {
            var espaciosdos = " ";
          }
        );
...
router.delete("/eliminarcuenta/:email", (req, res) => {
  var email = req.params.email;
  conexion.query(
    "DELETE from prayside_usuarios where email = ?",
    [email],
    (err, rows, fields) => {
      if (err) {
        console.log(err);
