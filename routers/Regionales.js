...
router.get("/buscarpaises", (req, res) => {
  conexion.query(
    "SELECT paiscodigodos, paisnombre FROM prayside_paises order by paisnombre",
    (err, rows, fields) => {
      if (err) {
        console.log(err);
      } else {
...
router.get("/buscarprovincias/:pais", (req, res) => {
  ...
  var sqltext =
    "SELECT paiscodigodos, provinciacodigo, provincianombre " +
...
router.get("/buscarciudades/:pais", (req, res) => {
  var pais = req.params.pais;
  var sqltext =
    "SELECT paiscodigodos, ciudadcodigo, ciudadnombre, provinciacodigo " +
